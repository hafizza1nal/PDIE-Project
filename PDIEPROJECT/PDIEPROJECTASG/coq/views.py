...
def booking1(request):
    ...
    return render (request,"booking1.html",dict)
...
def addevent(request):
    if request.method == 'POST':
        ...
        if event_name and event_description and event_date:
            # Check if event already exists
            if Event.objects.filter(Eventname=event_name).exists():
                messages.error(request, 'Event with this name already exists.')
            else:
                # Create new event
                Event.objects.create(Eventname=event_name, Eventdescription=event_description, EventDate=event_date)
...
def viewdeleteevent(request):
    alldata = Event.objects.all()
    dict={
        'alldata':alldata
    }
    return render (request,"viewdeleteevent.html",dict)

def delete_eventpage(request, Eventname):
    data = Event.objects.get(Eventname=Eventname)
    data.delete()
    return HttpResponseRedirect(reverse("viewdeleteevent"))
...
def addfacility(request):
    if request.method == 'POST':
        ...
        if facility_name:
            # Check if event already exists
            if Facility.objects.filter(Facilityname=facility_name).exists():
                messages.error(request, 'This facility already exists.')
            else:
                # Create new event
                Facility.objects.create(Facilityname=facility_name)
...
def viewdeletefacility(request):
    alldata = Facility.objects.all()
    dict={
        'alldata':alldata
    }
    return render (request,"viewdeletefacility.html",dict)

def delete_facilitypage(request, Facilityname):
    data = Facility.objects.get(Facilityname=Facilityname)
    data.delete()
    return HttpResponseRedirect(reverse("viewdeletefacility"))

def viewevent(request):
    alldata = Registration.objects.all()
    dict={
        'alldata':alldata
    }
    return render (request,"viewevent.html",dict)
...
def viewfacility(request):
    ...
    return render (request,"viewfacility.html",dict)